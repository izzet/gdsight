#!/usr/bin/env bash
# Reproduce the 3 curated GDS anti-patterns where standard tools mislead but GDSight gives the right
...
echo "  GDSight: unaligned byte-amp ~2x (device 2x requested) -> 'align data, ~$(awk "BEGIN{printf \"%.1f\",$AL/$UN}")x free'"
