# DeepNVMe (DeepSpeed) under GDSight
...
Both move the same device bytes (~1024 cmds), but only GDSight shows **which config is actually zero-copy
...
commands (overhead-bound → 2.46 GB/s). This is *why* the autotuner converges to ~1 MiB — and GDSight shows
...
- Each `cuFileRead` is a whole param tensor (~196 MB) → ~74 device commands; GDSight ties each of the
...
## Cross-check: current tools vs GDSight (measured, not asserted)
...
iostat: device-command count); **neither attributes per-op**. GDSight is the only view that spans both
...
On a real, autotuned production I/O layer + a real inference workload, GDSight: **(1)** traces the GDS path
